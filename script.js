...
const GameSettings = {
  ...
  init() {
    ...
    console.log(
      "GameSettings Initialized (will apply settings later)");
    ...
    if(savedSettings) {
      ...
      try {
        ...
        this.settings = {
          ...
          touchSensitivity: parseFloat(parsed.touchSensitivity ??
            this.settings.touchSensitivity),

          musicVolume: parseFloat(parsed.musicVolume ?? this
            .settings.musicVolume),

          sfxVolume: parseFloat(parsed.sfxVolume ?? this.settings
            .sfxVolume),

          // Ensure boolean is boolean

          useSpatialReverb: typeof parsed.useSpatialReverb ===
            'boolean' ? parsed.useSpatialReverb : this.settings
            .useSpatialReverb,
        ...
        'fpsGameSettings'); // Remove corrupted data
  ...
  saveSettings() {
    ...
    this.settings.language = document.getElementById(
      'languageSelect')?.value ?? this.settings.language;

    this.settings.forwardKey = document.getElementById(
      'forwardKeyInput')?.value ?? this.settings.forwardKey;

    this.settings.backwardKey = document.getElementById(
      'backwardKeyInput')?.value ?? this.settings.backwardKey;

    this.settings.leftKey = document.getElementById('leftKeyInput')
      ?.value ?? this.settings.leftKey;

    this.settings.rightKey = document.getElementById(
      'rightKeyInput')?.value ?? this.settings.rightKey;

    this.settings.touchSensitivity = parseFloat(document
      .getElementById('touchSensitivity')?.value ?? this.settings
      .touchSensitivity);

    this.settings.musicVolume = parseFloat(document.getElementById(
      'musicVolume')?.value ?? this.settings.musicVolume);

    this.settings.sfxVolume = parseFloat(document.getElementById(
      'sfxVolume')?.value ?? this.settings.sfxVolume);

    this.settings.useSpatialReverb = document.getElementById(
      'spatialReverb')?.checked ?? this.settings.useSpatialReverb;

    this.settings.reverbQuality = document.getElementById(
      'reverbQuality')?.value ?? this.settings.reverbQuality;

    try {

      localStorage.setItem('fpsGameSettings', JSON.stringify(this
        .settings));

      // console.log("Saved settings:", this.settings);

      this
    .applySettings(); // Apply settings immediately after saving/changing
  ...
  applySettings() {
    ...
    if(AudioManager.setMusicVolume) AudioManager.setMusicVolume(this
      .settings.musicVolume);

    if(AudioManager.setSfxVolume) AudioManager.setSfxVolume(this
      .settings.sfxVolume);

    if(AudioManager.setReverb) AudioManager.setReverb(this.settings
      .useSpatialReverb, this.settings.reverbQuality);

    if(InputManager.setTouchSensitivity) InputManager
      .setTouchSensitivity(this.settings.touchSensitivity);

    if(InputManager.updateKeyBindings) InputManager
      .updateKeyBindings(); // Tell InputManager to re-read key bindings

    // Language change might require a UI refresh or re-init of localization library

    // For now, just log the change

    console.log("Applied language setting:", this.settings
    .language);
  ...
  updateUIFromSettings() {
    ...
    const forwardKeyInput = document.getElementById(
      'forwardKeyInput');

    if(forwardKeyInput) forwardKeyInput.value = this.settings
      .forwardKey;

    const backwardKeyInput = document.getElementById(
      'backwardKeyInput');

    if(backwardKeyInput) backwardKeyInput.value = this.settings
      .backwardKey;
    ...
    const touchSensitivitySlider = document.getElementById(
      'touchSensitivity');

    if(touchSensitivitySlider) touchSensitivitySlider.value = this
      .settings.touchSensitivity;

    const touchSensitivityValue = document.getElementById(
      'touchSensitivityValue');

    if(touchSensitivityValue) touchSensitivityValue.textContent =
      parseFloat(this.settings.touchSensitivity).toFixed(1);

    const musicVolSlider = document.getElementById('musicVolume');

    if(musicVolSlider) musicVolSlider.value = this.settings
      .musicVolume;

    const musicVolumeValue = document.getElementById(
      'musicVolumeValue');

    if(musicVolumeValue) musicVolumeValue.textContent = Math.round(
    ...
    const sfxVolumeValue = document.getElementById(
    'sfxVolumeValue');

    if(sfxVolumeValue) sfxVolumeValue.textContent = Math.round(this
      .settings.sfxVolume * 100);

    const reverbCheckbox = document.getElementById('spatialReverb');

    if(reverbCheckbox) reverbCheckbox.checked = this.settings
      .useSpatialReverb;

    const reverbQualitySelect = document.getElementById(
      'reverbQuality');

    if(reverbQualitySelect) reverbQualitySelect.value = this
      .settings.reverbQuality;
  ...
  bindControlEvents() {
    ...
    document.querySelectorAll(
      '#settingsScreen select, #settingsScreen input[type="checkbox"], #settingsScreen input[type="radio"]'
      ).forEach(element => {
    ...
    document.querySelectorAll('#settingsScreen input[type="range"]')
      .forEach(element => {

        element.addEventListener('input', () => {

          this
        .saveSettings(); // Save continuously while sliding

          // Update displayed value next to slider

          const valueDisplay = document.getElementById(element
            .id + 'Value');
          ...
          if(valueDisplay) {
            ...
            if(element.id === 'touchSensitivity') {
              ...
              valueDisplay.textContent = parseFloat(element
                .value).toFixed(1);

            } else {

              valueDisplay.textContent = Math.round(element
                .value * 100);
    ...
    document.querySelectorAll('#settingsScreen input.keybind-input')
      .forEach(input => {

        input.addEventListener('keydown', (e) => {

          e
        .preventDefault(); // Prevent typing the key character itself

          input.value = e
          .code; // Display the code (e.g., "KeyW", "ArrowUp")

          this
        .saveSettings(); // Save immediately on key press display
...
const AudioManager = {
  ...
  init() {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.audioContext = new(window.AudioContext || window
          .webkitAudioContext)();
        ...
        this.listener = new THREE
      .AudioListener(); // Use THREE's listener
        ...
        this.masterGainNode.connect(this.audioContext
          .destination);

        console.log(
          "AudioManager Initializing - Loading Sounds...");
        ...
        console.error(
          "Web Audio API not supported or context creation failed.",
          e);
  ...
  async loadAllSounds() {
    ...
    if(!this.audioContext) return Promise
  .resolve(); // Skip loading if context failed
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        loader.load(path,
          ...
          (buffer) => {
            ...
            buffer; /*console.log(`Sound loaded: ${key}`);*/
            resolve();
          },
          ...
          (err) => {
            console.error(
              `Failed to load sound ${key} from ${path}:`,
              err);
            resolve(); /* Resolve anyway? Or reject? */
          }
    ...
    for(const key in this.reverbsToLoad) {
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        loader.load(path,
          ...
          (buffer) => {
            this.loadedReverbs[key] =
            buffer; /*console.log(`Reverb loaded: ${key}`);*/
            resolve();
          },
          ...
          (err) => {
            console.error(
              `Failed to load reverb IR ${key} from ${path}:`,
              err);
  ...
  playSound(soundKey, position = null, volume = 1.0, playbackRate =
    1.0, loop = false) {

    if(!this.audioContext || this.audioContext.state ===
      'suspended' || !this.loadedSounds[soundKey] || !this
      .sfxGainNode) {
    ...
    source.playbackRate.setValueAtTime(playbackRate, this
    ...
    individualGain.gain.setValueAtTime(volume, this.audioContext
      .currentTime);
    ...
    if(position && this.listener && typeof position.x ===
      'number') { // Check if position is valid
      ...
      if(panner.positionX) {
        ...
        panner.positionX.setValueAtTime(position.x, this
          .audioContext.currentTime);

        panner.positionY.setValueAtTime(position.y, this
          .audioContext.currentTime);

        panner.positionZ.setValueAtTime(position.z, this
          .audioContext.currentTime);
      ...
      soundNodeChain =
      panner; // Panner is now end of the main chain for direct sound

      // Reverb Path (if enabled and IR loaded for current quality)

      if(this.reverbEnabled && this.currentReverbNode && this
        .loadedReverbs[this.reverbQuality]) {

        const reverbSendGain = this.audioContext.createGain();

        // Adjust reverb send level (e.g., less than direct path volume)

        reverbSendGain.gain.setValueAtTime(volume * 0.35, this
          .audioContext.currentTime); // Example: 35% send
  ...
  playBGM() {
    ...
    if(!this.audioContext || this.audioContext.state ===
      'suspended' || !this.loadedSounds['bgm'] || !this
      .musicGainNode) {
  ...
  setMusicVolume(volume) {
    ...
    if(this.musicGainNode && this.audioContext) {
      ...
      this.musicGainNode.gain.setTargetAtTime(clamp(volume, 0, 1),
        this.audioContext.currentTime, 0.05); // Smooth ramp
  ...
  setSfxVolume(volume) {
    ...
    if(this.sfxGainNode && this.audioContext) {
      ...
      this.sfxGainNode.gain.setTargetAtTime(clamp(volume, 0, 1),
        this.audioContext.currentTime, 0.05); // Smooth ramp
  ...
  setReverb(enabled, quality) {
    ...
    if(enabled && this.loadedReverbs[quality]) {
      ...
      if(!this.currentReverbNode || this.currentReverbNode
        .buffer !== this.loadedReverbs[quality]) {

        // Create new convolver only if it doesn't exist or quality changed

        this.currentReverbNode = this.audioContext
      .createConvolver();

        this.currentReverbNode.buffer = this.loadedReverbs[quality];

        console.log(
          `Reverb Enabled: Created/Set Convolver Quality - ${quality}`
          );
      ...
      if(!this.loadedReverbs[quality]) {
        ...
        console.warn(
          `Reverb IR for quality "${quality}" not loaded. Disabling reverb.`
          );

        this.reverbEnabled =
        false; // Force disable if IR is missing

        // Optionally update UI to reflect this forced state

        const reverbCheckbox = document.getElementById(
          'spatialReverb');
        ...
        if(reverbQualitySelect) reverbQualitySelect.disabled =
        true; // Disable quality select if IR missing?
...
const Graphics = {
  ...
  init() {
    ...
    0); // Position relative to pitch object
    ...
    this.renderer = new THREE.WebGLRenderer({
      ...
      antialias: true,
      alpha: false
    }); // Ensure alpha is false unless needed
    ...
    .sRGBEncoding; // Correct color space
    ...
    if(audioListener) {
      ...
      this.camera.add(
      audioListener); // Add the listener used by AudioManager
      ...
      console.warn(
        "Audio Listener not available when Graphics init ran.");

    }

    // Resize listener

    window.addEventListener('resize', this.onWindowResize.bind(
      this));
  ...
  setupLighting() {
    ...
    100; // Adjusted shadow camera range
    ...
    0.002; // Adjust bias carefully if shadow acne occurs

    this.scene.add(directionalLight);

    this.scene.add(directionalLight
    .target); // Target defaults to (0,0,0), can be moved if needed
  ...
  attachCameraToPlayer(playerMesh) {
    ...
    if(!playerMesh) {
      ...
      console.error(
        "Cannot attach camera: Provided player mesh is invalid.");
    ...
    if(!this.cameraPitchObject) {
      ...
      console.error(
        "Cannot attach camera: cameraPitchObject is not initialized."
        );
    ...
    this.cameraYawObject =
    playerMesh; // Yaw is controlled by the mesh linked to the physics body
    ...
    this.cameraYawObject.add(this
    .cameraPitchObject); // Add pitch control to the player mesh
  ...
  updateCameraRotation(targetYaw, targetPitch) {
    ...
    this.cameraPitchObject.rotation.x = lerp(this.cameraPitchObject
      .rotation.x, targetPitch, lerpFactor);

    // Clamp pitch immediately after lerping

    this.cameraPitchObject.rotation.x = clamp(this.cameraPitchObject
      .rotation.x, -Math.PI / 2 + 0.1, Math.PI / 2 - 0.1);
  ...
  render(deltaTime) {
    ...
    this.updateCameraRotation(InputManager.targetYaw, InputManager
      .targetPitch);
...
const Physics = {
  ...
  init() {
    ...
    12; // Increase solver iterations slightly
    ...
    if(this.player.mesh) {
      ...
      console.error(
        "Player mesh not created, cannot attach camera!");
  ...
  setupContactMaterials() {
    ...
    defaultContactMaterial; // Set as default
  ...
  createGroundPlane() {
    ...
    0.01; // Slightly below 0 to avoid z-fighting if player starts exactly at y=0
  ...
  createPlayerBody() {
    ...
    this.player.shape = new CANNON.Box(new CANNON.Vec3(this.player
      .radius, this.player.height / 2, this.player.radius));

    this.player.body = new CANNON.Body({

      mass: 75,

      position: new CANNON.Vec3(0, 5,
      5), // Start higher to avoid initial ground intersection issues
    ...
    const playerGeometry = new THREE.CapsuleGeometry(this.player
      .radius, this.player.height - this.player.radius * 2, 8, 16);
    ...
    this.player.mesh = new THREE.Mesh(playerGeometry,
      playerMaterial);
    ...
    Graphics.addObject(this.player
    .mesh); // Add mesh to the graphics scene

    // Link body and mesh via userData

    this.player.body.userData = {
      mesh: this.player.mesh,
      isPlayer: true
    };

    this.player.mesh.userData = {
    ...
    this.player.body.addEventListener("collide", (event) => {
      ...
      if(otherBody.material === this.materials.ground) {
        ...
        if(impactVelocity >
          1.5) { // Threshold for footstep sound

          // AudioManager.playSound('footstep', contact.rj, 0.4); // Play at contact point

        }

      } else if(otherBody.userData
        ?.targetData) { // Check for targetData in userData
  ...
  update(deltaTime) {
    ...
    const maxSubSteps =
    5; // Allow more substeps if needed for stability
  ...
  applyPlayerMovementAndRotation(deltaTime) {
    ...
    if(!this.player.body || (!Game.gameStarted || Game.isPaused ||
        Game.isSettingsOpen)) {
    ...
    const acceleration =
    40.0; // How quickly player reaches moveSpeed (force multiplier)

    const inputVector = InputManager
  .getMovementVector(); // {x, z} normalized direction
    ...
    this.player.body.quaternion.vmult(forwardDir,
    forwardDir); // Rotate Vec3 by Quaternion
    ...
    desiredVel.scale(moveSpeed,
    desiredVel); // Scale to target speed

    // Calculate velocity change needed

    const currentVelXZ = new CANNON.Vec3(this.player.body.velocity
      .x, 0, this.player.body.velocity.z);
    ...
    const currentSpeedSq = this.player.body.velocity.x ** 2 + this
    ...
    targetQuaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0),
      targetYaw);

    // Smoothly interpolate the physics body's quaternion towards the target

    const lerpFactor =
    0.2; // Smoothing factor for rotation (adjust as needed)

    currentQuaternion.slerp(targetQuaternion, lerpFactor, this
  ...
  syncMeshes() {
    ...
    TargetManager.targets.forEach(targetData => {
      ...
      if(targetData.mesh && targetData.body) {
        ...
        targetData.mesh.quaternion.copy(targetData.body
          .quaternion);
        ...
        if(targetData.body.position.y < -
          15) { // Lower threshold

          // console.log(`Target ${targetData.id} fell out of bounds.`);

          TargetManager.removeTarget(targetData,
          true); // Mark as out of bounds
  ...
  raycast(originVec3, directionVec3, distance) {
    ...
    const origin = new CANNON.Vec3(originVec3.x, originVec3.y,
      originVec3.z);

    const direction = new CANNON.Vec3(directionVec3.x, directionVec3
      .y, directionVec3.z);
    ...
    to.vadd(direction.scale(distance),
    to); // Add scaled direction vector
  ...
  removeBody(body) {
    ...
    if(body.userData?.mesh) {
      ...
      mesh); // Use Graphics module to remove and dispose

      if(mesh.userData) {

        mesh.userData.body =
        null; // Break link from mesh back to body
...
const InputManager = {
  ...
  mouse: {
    ...
    dx: 0,
    dy: 0, // Delta movement this frame
  ...
  touch: {
    ...
    moveDeltaX: 0,
    moveDeltaY: 0, // Raw delta from start for joystick visual
    ...
    lookDeltaX: 0,
    lookDeltaY: 0, // Delta for look rotation calculation this frame
  ...
  resetState() {
    ...
    this.touch = {
      ...
      .sensitivity, // Keep loaded sensitivity
  ...
  setTouchSensitivity(sensitivity) {
    ...
    this.touch.sensitivity = clamp(sensitivity, 0.1,
    3.0); // Clamp sensitivity range
  ...
  addEventListeners() {
    ...
    document.addEventListener('pointerlockchange', this
      .handlePointerLockChange.bind(this), false);

    document.addEventListener('mozpointerlockchange', this
      .handlePointerLockChange.bind(this), false); // Firefox

    // Click on canvas to request pointer lock

    Graphics.renderer.domElement.addEventListener('click', () => {

      if(!isMobile && !this.pointerLocked && Game.gameStarted &&
        !Game.isPaused && !Game.isSettingsOpen) {

        Graphics.renderer.domElement.requestPointerLock()

          .catch(err => console.error(
            "Failed to request pointer lock:", err));
    ...
    window.addEventListener('mousemove', this.handleMouseMove.bind(
      this));
    ...
    Graphics.renderer.domElement.addEventListener('pointerdown',
      this.handlePointerDown.bind(this));

    Graphics.renderer.domElement.addEventListener('pointerup', this
      .handlePointerUp.bind(this));

    // Prevent context menu on right click over canvas

    Graphics.renderer.domElement.addEventListener('contextmenu', (
      e) => e.preventDefault());
  ...
  handleKeyDown(e) {
    ...
    if(Game.isSettingsOpen && !e.target.classList.contains(
        'keybind-input')) return;
  ...
  handlePointerLockChange() {
    ...
    const lockElement = document.pointerLockElement || document
    ...
    if(lockElement === Graphics.renderer.domElement) {
      ...
      if(this
        .pointerLocked) { // Only trigger if lock was previously held
        ...
        if(Game.gameStarted && !Game.isPaused && !Game
          .isSettingsOpen) {

          console.log(
          'Pointer lock lost during gameplay, pausing.');
  ...
  handleMouseMove(event) {
    ...
    if(!this.pointerLocked || !Game.gameStarted || Game.isPaused ||
      Game.isSettingsOpen) {

      this.mouse.dx = 0;
      this.mouse.dy = 0; // Reset delta if not active
    ...
    this.targetPitch = clamp(this.targetPitch, -Math.PI / 2 + 0.1,
      Math.PI / 2 - 0.1);
  ...
  addMobileTouchListeners() {
    ...
    const canvas = Graphics.renderer
    .domElement; // Use canvas for look input

    // --- Touch Start ---

    document.body.addEventListener('touchstart', (
    e) => { // Listen on body to capture all touches initially

      if(!Game.gameStarted || Game.isPaused || Game
        .isSettingsOpen) return;

      // e.preventDefault(); // Prevent default actions like scrolling/zooming ONLY if touch is handled

      const joystickRect = joystickArea
    ?.getBoundingClientRect();

      const shootButtonRect = shootButton
        ?.getBoundingClientRect();
      ...
      for(let i = 0; i < e.changedTouches.length; i++) {
        ...
        if(joystickArea && this.touch.moveIdentifier === null &&
          ...
          touchX >= joystickRect.left && touchX <= joystickRect
          .right &&

          touchY >= joystickRect.top && touchY <= joystickRect
          .bottom)
        ...
        else if(shootButton && this.touch.shootIdentifier ===
          null &&

          touchX >= shootButtonRect.left && touchX <=
          shootButtonRect.right &&

          touchY >= shootButtonRect.top && touchY <=
          shootButtonRect.bottom)

        {
          ...
          this.mouse.leftButton =
          true; // Simulate left mouse down
        ...
        // Check if touch is for Looking (right side of screen, not on buttons)
        ...
        else if(this.touch.lookIdentifier === null && touchX >
          window.innerWidth / 2) {
          ...
          if(!onShootButton) {
            ...
            this.touch.lookDeltaX =
            0; // Reset delta for the frame
    ...
    document.body.addEventListener('touchmove', (e) => {
      ...
      for(let i = 0; i < e.changedTouches.length; i++) {
        ...
        if(touch.identifier === this.touch.moveIdentifier ||
          touch.identifier === this.touch.lookIdentifier) {
      ...
      for(let i = 0; i < e.changedTouches.length; i++) {
        ...
        if(touch.identifier === this.touch.moveIdentifier) {
          ...
          const visualDeltaX = this.touch.moveCurrentX - this
            .touch.moveStartX;

          const visualDeltaY = this.touch.moveCurrentY - this
            .touch.moveStartY;
        ...
        // Handle Look Drag
        ...
        else if(touch.identifier === this.touch
          .lookIdentifier) {
          ...
          this.touch.lookDeltaX =
          deltaX; // Store delta for this frame

          this.touch.lookDeltaY = deltaY;

          // Update target angles (apply sensitivity here)

          const effectiveSensitivity = this.lookSensitivity *
            this.mobileLookSensitivityMultiplier * this.touch
            .sensitivity;
          ...
          this.targetPitch = clamp(this.targetPitch, -Math.PI /
            2 + 0.1, Math.PI / 2 - 0.1);
    ...
    const handleTouchEnd = (e) => {
      ...
      if(!Game.gameStarted)
    return; // Check game started, but allow ending touches even if paused/settings
    ...
    handleTouchEnd); // Handle interruptions
  ...
  updateJoystickVisual(deltaX, deltaY) {
    ...
    const maxDist = base.offsetWidth /
    3; // Max distance knob moves from center

    // Clamp the delta vector magnitude

    const currentDist = Math.sqrt(deltaX * deltaX + deltaY *
    deltaY);
  ...
  handlePointerDown(event) {
    ...
    if(event.target !== Graphics.renderer.domElement && event
      .target !== document.body && event.target !== document
      .documentElement) {
    ...
    if(isMobile && event.pointerType === 'touch') {
      ...
      if(event.pointerId === this.touch.moveIdentifier || event
        .pointerId === this.touch.lookIdentifier) {
  ...
  getMovementVector() {
    ...
    if(isMobile) {
      ...
      if(this.touch.moveActive && this.touch.moveIdentifier !==
        null) {

        const maxDist = document.getElementById('joystick')
          ?.offsetWidth / 3 ?? 50; // Use base radius
        ...
        const dy = this.touch
        .moveDeltaY; // Raw delta from start pos
        ...
        if(dist > 0) {
          ...
          const normalizedY = clamp(dy / maxDist, -1,
          1); // Y delta corresponds to Z movement

          vector.x = normalizedX;

          vector.z = -
          normalizedY; // Invert Y axis: Up on joystick (-Y) means forward (-Z)
    ...
    const length = Math.sqrt(vector.x * vector.x + vector.z * vector
      .z);
...
const TargetManager = {
  ...
  cacheTargetAssets() {
    ...
    Object.values(this.targetTypes).forEach(type => {
      ...
      if(!this.targetGeometryCache[geoKey]) {
        ...
        this.targetGeometryCache[geoKey] = new THREE
          .BoxGeometry(
      ...
      if(!this.targetMaterialCache[matKey]) {
        ...
        this.targetMaterialCache[matKey] = new THREE
  ...
  reset() {
    ...
    [...this.targets].forEach(targetData => this.removeTarget(
      targetData)); // Iterate over a copy

    this.targets = []; // Ensure array is empty

    this.timeSinceLastSpawn = 0;

    this.nextSpawnTime = this.spawnIntervalBase /
    2; // Spawn first target relatively quickly
  ...
  update(deltaTime) {
    ...
    if(this.targets.length < this.maxTargets && this
      .timeSinceLastSpawn >= this.nextSpawnTime) {
      ...
      this.nextSpawnTime = this.spawnIntervalBase + (Math.random() -
        0.5) * 2 * this.spawnIntervalVariance;

      this.nextSpawnTime = Math.max(0.4, this
      .nextSpawnTime); // Ensure minimum spawn delay
  ...
  spawnTarget() {
    ...
    const randomTypeKey = typeKeys[Math.floor(Math.random() *
      typeKeys.length)];
    ...
    if(!geometry || !material) {
      ...
      console.error(
        `Could not find cached assets for target type: ${type.name}`
        );
    ...
    mesh.receiveShadow =
    false; // Targets usually don't receive complex shadows

    // --- Spawn Position & Rotation ---

    const spawnX = THREE.MathUtils.randFloat(this.spawnArea.minX,
      this.spawnArea.maxX);

    const spawnY = THREE.MathUtils.randFloat(this.spawnArea.minY,
      this.spawnArea.maxY);

    const spawnZ = THREE.MathUtils.randFloat(this.spawnArea.minZ,
      this.spawnArea.maxZ);

    mesh.position.set(spawnX, spawnY, spawnZ);

    mesh.rotation.y = Math.random() * Math.PI *
    2; // Random initial yaw
    ...
    const body = new CANNON.Body({
      ...
      mass: type.massMultiplier * type.scale *
      1.5, // Mass based on type and scale
    ...
    const randomImpulse = new CANNON.Vec3(
      ...
      0.5, // Less upward impulse

      (Math.random() - 0.5) * impulseStrength

    );

    const randomPoint = new CANNON
    .Vec3( // Apply impulse off-center for rotation
    ...
    }; // Overwrite/set userData

    mesh.userData = {
      targetId: targetId
    }; // Store ID on mesh for potential debugging
    ...
    false); // Add body, mesh linking handled above
  ...
  getTargetDataFromBody(body) {
    ...
    ?.targetData; // Safely access nested property
  ...
  processHit(hitBody, hitPoint, hitNormal, damage = 1) {
    ...
    if(!targetData || targetData.currentHealth <= 0) {
      ...
      return {
        ...
        wasPenalized: false,
        wasPenetrable: false
      }; // Target already gone or invalid
    ...
    damage; // Scale impulse with damage

    const impulse = new CANNON.Vec3(-hitNormal.x, -hitNormal.y, -
      hitNormal.z); // Impulse opposite to normal

    impulse.scale(impulseMagnitude, impulse);

    hitBody.applyImpulse(impulse,
    hitPoint); // Apply impulse at the hit point
    ...
    let wasPenetrable = targetData.type
    .penetrable; // Return if the hit target was penetrable
    ...
    if(targetData.currentHealth <= 0) {
      ...
      score = targetData.type.points >= 0 ? Math.round(targetData
        .type.points * 0.1) : 0; // e.g., 10% points if not penalty

      wasPenalized =
      false; // Not fully penalized if not destroyed? Or penalize per hit?

      Game.registerHit(targetData.type.points,
      false); // Register partial hit (less/no points/combo)
    ...
    return {
      ...
      wasPenalized,
      wasPenetrable
    }; // Return info about the hit result
  ...
  removeTarget(targetDataOrId, isOutOfBounds = false) {
    ...
    const idToRemove = (typeof targetDataOrId === 'string') ?
      targetDataOrId : targetDataOrId?.id;
    ...
    if(index > -1) {
      ...
      if(isOutOfBounds && targetData.type.points >
        0) { // Only penalize if it wasn't a penalty target already
...
const Game = {
  ...
  uiElements: {
    ...
    restartButton: null,
    resumeButton: null,
    quitButton: null, // Example buttons
  ...
  init() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        await AudioManager
      .init(); // Needs to load sounds (async)

        if(!Graphics.init()) throw new Error(
          "Graphics initialization failed"
          ); // Needs Audio Listener

        if(!Physics.init()) throw new Error(
          "Physics initialization failed"
          ); // Needs Graphics, creates player mesh

        TargetManager.init(); // Needs Physics materials

        GameSettings.init(); // Load settings data

        if(!InputManager.init()) throw new Error(
          "Input Manager initialization failed"
          ); // Needs Graphics, Physics, Settings

        // Cache DOM elements AFTER basic init

        this.cacheDOMElements();

        if(!this.uiElements.gameCanvas || !this.uiElements
          .settingsScreen) {

          throw new Error(
            "Essential UI elements not found in HTML.");
        ...
        console.log(
          "Game Initialized Successfully. Ready for Start.");
  ...
  cacheDOMElements() {
    ...
    this.uiElements.gameCanvas = document.getElementById(
      'gameCanvas'); // Essential check

    this.uiElements.score = document.getElementById(
    'scoreValue'); // Target the value span

    this.uiElements.combo = document.getElementById('combo');

    this.uiElements.comboLabel = document.getElementById(
      'comboLabel');

    this.uiElements.comboValue = document.getElementById(
      'comboValue');

    this.uiElements.comboHits = document.getElementById(
    'comboHits');

    this.uiElements.ammo = document.getElementById(
    'ammoValue'); // Target the value span

    this.uiElements.maxAmmo = document.getElementById(
      'ammoMaxValue'); // Target the value span

    this.uiElements.totalAmmo = document.getElementById(
      'totalAmmoValue'); // Target the value span

    this.uiElements.reloadIndicator = document.getElementById(
      'reloadIndicator');

    this.uiElements.timer = document.getElementById(
    'timerValue'); // Target the value span

    this.uiElements.healthFill = document.getElementById(
      'healthFill');

    this.uiElements.healthValue = document.getElementById(
      'healthValue'); // Target the value span

    this.uiElements.hitMarker = document.getElementById(
    'hitMarker');

    this.uiElements.gunEffects = document.getElementById(
      'gunEffects'); // For muzzle flash, etc.

    this.uiElements.crosshair = document.getElementById(
    'crosshair');

    this.uiElements.gameUI = document.getElementById('gameUI');

    this.uiElements.settingsButton = document.getElementById(
      'settingsButton');

    this.uiElements.settingsScreen = document.getElementById(
      'settingsScreen'); // Essential check

    this.uiElements.pauseScreen = document.getElementById(
      'pauseScreen');

    this.uiElements.gameOverScreen = document.getElementById(
      'gameOverScreen');

    this.uiElements.finalScore = document.getElementById(
      'finalScoreValue'); // Target the value span

    this.uiElements.maxComboStat = document.getElementById(
      'maxComboValue'); // Target the value span

    this.uiElements.startScreen = document.getElementById(
      'startScreen');

    this.uiElements.startButton = document.getElementById(
      'startButton');

    this.uiElements.restartButton = document.getElementById(
      'restartButton'); // In game over screen

    this.uiElements.resumeButton = document.getElementById(
      'resumeButton'); // In pause screen

    this.uiElements.quitButton = document.getElementById(
      'quitButton'); // In pause screen (optional)

    this.uiElements.mobileControls = document.getElementById(
      'mobileControls');
  ...
  setupUIInteractions() {
    ...
    this.uiElements.resumeButton?.addEventListener('click', () =>
      this.pauseGame(false)); // Pass false to unpause
  ...
  resetGame() {
    ...
    this.timeSinceLastShot = this
    .shootCooldown; // Ready to shoot immediately
    ...
    InputManager
  .resetState(); // Reset input states (keys, touch, look angles)
    ...
    if(Physics.player.body) {
      ...
      Physics.player.body.quaternion.set(0, 0, 0,
      1); // Identity quaternion (facing Z-)
      ...
      if(Graphics.cameraPitchObject) Graphics.cameraPitchObject
        .rotation.x = 0;
    ...
    if(this.uiElements.reloadIndicator) this.uiElements
    ...
    if(this.uiElements.gameUI) this.uiElements.gameUI.style
      .display = 'none'; // Hide game UI initially

    if(this.uiElements.startScreen) this.uiElements.startScreen
      .style.display = 'flex'; // Show start screen

    if(this.uiElements.settingsButton) this.uiElements
      .settingsButton.classList.add('hidden'); // Hide settings btn

    if(this.uiElements.mobileControls) this.uiElements
      .mobileControls.style.display = isMobile ? 'block' :
      'none'; // Show mobile controls if needed
  ...
  startGame() {
    ...
    if(!isMobile) {
      ...
      Graphics.renderer.domElement.requestPointerLock()
        ...
        .catch(err => console.warn(
          "Could not acquire pointer lock on start:", err));
    ...
    TargetManager.timeSinceLastSpawn = TargetManager
      .spawnIntervalBase; // Spawn one relatively quickly
  ...
  pauseGame(pauseState) {
    ...
    if(this.isGameOver || this.isSettingsOpen)
  return; // Don't pause if game over or settings open

    if(this.isPaused === pauseState)
  return; // Already in the desired state
    ...
    if(this.isPaused) {
      ...
      AudioManager.setMusicVolume(GameSettings.settings
        .musicVolume * 0.3); // Lower music volume
      ...
      AudioManager.setMusicVolume(GameSettings.settings
      .musicVolume); // Restore music volume
      ...
      if(!isMobile) {
        ...
        Graphics.renderer.domElement.requestPointerLock()
          ...
          .catch(err => console.warn(
            "Could not acquire pointer lock on resume:", err));
  ...
  showSettings() {
    ...
    this.wasPausedBeforeSettings = this
    .isPaused; // Remember if game was already paused

    this.isPaused = true; // Set internal pause state

    this.showSettingsScreen();

    AudioManager.setMusicVolume(GameSettings.settings.musicVolume *
      0.3); // Lower music volume
  ...
  hideSettings() {
    ...
    if(!this.isPaused && !this.isGameOver && this.gameStarted && !
      ...
      Graphics.renderer.domElement.requestPointerLock()
        ...
        .catch(err => console.warn(
          "Could not acquire pointer lock after closing settings:",
          err));
  ...
  showPauseScreen() {
    if(this.uiElements.pauseScreen) this.uiElements.pauseScreen
      .style.display = 'flex';
  },
  ...
  showGameOverScreen() {
    ...
    if(this.uiElements.gameOverScreen) {
      ...
      if(this.uiElements.finalScore) this.uiElements.finalScore
        .textContent = this.score;

      if(this.uiElements.maxComboStat) this.uiElements.maxComboStat
        .textContent = this.maxConsecutiveHits;
  ...
  hideGameOverScreen() {
    if(this.uiElements.gameOverScreen) this.uiElements
      .gameOverScreen.style.display = 'none';
  },

  showSettingsScreen() {
    if(this.uiElements.settingsScreen) this.uiElements
  ...
  gameLoop(timestamp) {
    ...
    requestAnimationFrame(this.gameLoop.bind(
    this)); // Schedule next frame
  ...
  updateGameLogic(deltaTime) {
    ...
    if(InputManager.isKeyDown(InputManager.keyBindings.reload) && !
      this.isReloading) {
  ...
  triggerShoot() {
    ...
    if(this.isReloading || this.timeSinceLastShot < this
      .shootCooldown) {
    ...
    AudioManager.playSound('shoot', null,
    0.8); // Play shoot sound non-spatially
    ...
    const hits = Physics.raycast(rayOrigin, rayDirection,
    300); // Max distance 300 units

    // Process Hits (Penetration Logic)

    let penetrationPower = 1.0; // How much the bullet can penetrate

    const penetrationReduction =
    0.7; // Power reduction per penetrable layer
    ...
    if(hits.length > 0) {
      ...
      for(const hit of hits) {
        ...
        const hitResult = TargetManager.processHit(hitBody,
          hitPoint, hitNormal, 1); // Process hit with 1 damage

        if(
          hitResult) { // processHit should return an object if it hit a target
          ...
          if(hitResult.wasPenetrable && !hitResult.destroyed) {
            ...
            penetrationPower -=
            penetrationReduction; // Reduce power

          } else {

            penetrationPower =
            0; // Stop on non-penetrable or destroyed target

          }

          if(penetrationPower <= 0)
        break; // Stop looping if bullet stopped
  ...
  registerHit(pointsValue, wasDestroyed) {
    ...
    if(pointsValue > 0) {
      ...
      if(
        wasDestroyed) { // Only count combo/score multiplier if destroyed

        this.consecutiveHits++;

        this.maxConsecutiveHits = Math.max(this.maxConsecutiveHits,
          this.consecutiveHits);

        this.updateCombo();

        const pointsEarned = Math.round(pointsValue * this
          .comboMultiplier);
        ...
        const partialPoints = Math.round(pointsValue *
        0.1); // 10% points for damage?
    ...
    // Handle Penalty Targets separately
      ...
      console.log(
        `Penalty Hit! Score ${pointsValue}. Combo Reset. Health: ${this.health}`
        );
    ...
    this
  .updateComboUI(); // Update combo UI even on penalty hit (to show it resetting)
  ...
  registerMiss() {
    ...
    AudioManager.playSound('miss', null,
    0.6); // Non-spatial miss sound
  ...
  updateCombo() {
    ...
    for(const hits in this.comboThresholds) {
      ...
      if(this.consecutiveHits >= hitCount) {
        ...
        highestThreshold =
        hitCount; // Keep track for UI potentially
  ...
  startReload() {
    ...
    if(this.isReloading || this.ammo === this.maxAmmo || this
      .totalAmmo <= 0) {
  ...
  return; // Should not happen, but safety check
    ...
    if(ammoToLoad > 0) {
      ...
      console.log(
        `Reload Finished. Ammo: ${this.ammo}/${this.totalAmmo}`);
  ...
  return; // No damage if game over or amount is zero/negative
    ...
    console.log(
      `Player took ${amount} damage. Health: ${this.health}`);
  ...
  playerHeal(amount) {
    ...
    this.health = Math.min(this.maxHealth, this
    .health); // Clamp at max
  ...
  updateScoreUI() {
    ...
    if(this.uiElements.score) this.uiElements.score.textContent =
      this.score;
  ...
  updateComboUI() {
    ...
    if(this.uiElements.combo) {
      ...
      if(this.consecutiveHits > 0) {
        ...
        if(this.uiElements.comboValue) this.uiElements.comboValue
          .textContent = `${this.comboMultiplier.toFixed(1)}x`;

        if(this.uiElements.comboHits) this.uiElements.comboHits
          .textContent = `${this.consecutiveHits} HIT`;

        this.uiElements.combo.style.opacity =
        '1'; // Show combo display smoothly

        this.uiElements.combo.style.transform =
        'translateY(0)'; // Slide in?
  ...
  updateAmmoUI() {
    ...
    if(this.uiElements.ammo) this.uiElements.ammo.textContent = this
      .ammo;

    if(this.uiElements.maxAmmo) this.uiElements.maxAmmo
      .textContent = this.maxAmmo;

    if(this.uiElements.totalAmmo) this.uiElements.totalAmmo
      .textContent = this.totalAmmo;
  ...
  updateHealthUI() {
    ...
    if(this.uiElements.healthFill && this.uiElements.healthValue) {
      ...
      this.uiElements.healthValue.textContent = Math.round(this
        .health); // Show numerical health

      // Change color based on health percentage

      if(percentage < 25) {

        this.uiElements.healthFill.style.backgroundColor =
        '#ff4d4d'; // Red
  ...
  showHitMarker() {
    ...
    this.uiElements.hitMarker.style.transform =
      'translate(-50%, -50%) scale(1)'; // Reset scale

    // Add a quick scale animation

    requestAnimationFrame(
  () => { // Ensure style is applied before animating
    ...
    this.hitMarkerTimeoutId = setTimeout(() => {
      ...
      if(this.uiElements.hitMarker) {
        ...
        this.uiElements.hitMarker.style.transform =
          'translate(-50%, -50%) scale(0.8)'; // Shrink out
  ...
  showGunEffect(effectType) {
    ...
    if(effectType === 'muzzleFlash' && this.uiElements.gunEffects) {
      ...
      flash.className =
      'muzzle-flash'; // CSS handles appearance and animation

      this.uiElements.gunEffects.appendChild(flash);

      setTimeout(() => flash.remove(),
      100); // Remove after animation duration
  ...
  updateReloadIndicator() {
    ...
    if(!this.uiElements.reloadIndicator || !this.isReloading)
  return;

    const fillElement = this.uiElements.reloadIndicator
      .querySelector('.reload-fill');

    if(fillElement) {

      const progress = clamp(1 - (this.reloadTime), 0,
      1); // Calculate progress 0-1

      fillElement.style.width =
      `${progress * 100}%`; // Set fill width
  ...
  initializeAndStartGame() {
    ...
    Game.init().then(() => {
      ...
      Game
    .startGame(); // Only start game after full initialization

      console.log(
        "Game Initialized and Started via external call.");
...
console.log(
  "Main script.js loaded, Game object defined. Waiting for external start trigger."
  );
